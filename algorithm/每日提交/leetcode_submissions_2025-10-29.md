    ...
    for (let i = 0; i < n; i++) {
        ...
        for (let j = 0; j < n; j++) {
            ...
            for (let k = j + 1; k < n; k++) {
                ...
                const d1 = distance(p1, p3), d2 = distance(p1, p2), d3 = distance(p2, p3);
                // 判断是否直角三角形
                if (d3 == d1 + d2) {
    ...
    for (let i = 0; i < n - 3; i++) {
        for (let j = i + 1; j < n - 2; j++) {
            for (let k = j + 1; k < n - 1; k++) {
                for (let m = k + 1; m < n; m++) {
                    ans = Math.min(ans, isRect(points[i], points[j], points[k], points[m]))
                }
            }
        }
    }
...
const isRect = (p1, p2, p3, p4) => {
    const p = [p1, p2, p3, p4];
    let d1 = distance(p[0], p[1]), d2 = distance(p[0], p[2]), d3 = distance(p[3], p[1]),
        d4 = distance(p[3], p[2]), d5 = distance(p[0], p[3]), d6 = distance(p[1], p[2]);
...
##### 对点进行排序后计算距离，判断四条边是否相等且两条边和对应的对角线是否为直角a^2+b^2 = c^2
...
var validSquare = function (p1, p2, p3, p4) {
    ...
    let d1 = distance(p[0], p[1]), d2 = distance(p[0], p[2]),
        d3 = distance(p[3], p[1]), d4 = distance(p[3], p[2]),
        d5 = distance(p[0], p[3]);
...
var computeArea = function(ax1, ay1, ax2, ay2, bx1, by1, bx2, by2) {
    let unionx = Math.max(0, Math.min(ax2, bx2) - Math.max(ax1, bx1)), uniony =  Math.max(0, Math.min(ay2, by2) - Math.max(ay1, by1));
    return (ax2 - ax1) * (ay2 - ay1) + (bx2 - bx1) * (by2 - by1) - unionx * uniony;
};
```
...
var isRectangleOverlap = function (rec1, rec2) {
    return Math.max(rec1[1], rec2[1]) < Math.min(rec1[3], rec2[3]) && Math.min(rec1[2], rec2[2]) > Math.max(rec1[0], rec2[0])
};
```
