...
##### 贪心：维护保存下来的前一列的切割信息（如果a < b，表示要进行切割，新的一列比较时，不需要比较不同组的大小），只需要比较最新的一列的组内信息
...
var minDeletionSize = function (strs) {
    ...
    const cuts = Array(n).fill(false);
    let ans = 0;
    outer: for (let j = 0; j < m; j++) {
        let flag = true;
        for (let i = 1; i < n; i++) {
            if (!cuts[i] && strs[i - 1][j] > strs[i][j]) {
                ans++;
                continue outer;
...
var minDeletionSize = function (strs) {
    if (strs[0].length == 0) return 0;
    const n = strs.length, m = strs[0].length;
    const a = Array(n).fill(""); // 最终得到的字符串数组，当出现 ab ac db dc的时候比对字典序
    let ans = 0;
    outer: for (let j = 0; j < m; j++) {
        let flag = true;
        for (let i = 1; i < n; i++) {
            if (a[i - 1] + strs[i - 1][j] > a[i] + strs[i][j]) {
                ans++;
                continue outer;
            } else if (a[i - 1] + strs[i - 1][j] == a[i] + strs[i][j]) {
                flag = false;
            }
        }
        if (flag) break;
