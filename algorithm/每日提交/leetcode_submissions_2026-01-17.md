...
#### [3047. 求交集区域内的最大正方形面积](https://leetcode.cn/problems/find-the-largest-area-of-square-inside-two-rectangles/description/)
...
var largestSquareArea = function(bottomLeft, topRight) {
    ...
    let ans = 0;
    for (let i = 0; i < n; i++) {
        const bl = bottomLeft[i], tr = topRight[i];
        for (let j = 0; j < n; j++) {
            if (i == j) continue;
            const bl2 = bottomLeft[j], tr2 = topRight[j];
            let b1 = bl[0], b2 = bl2[0], t1 = tr[0], t2 = tr2[0];
            let l1 = bl[1], l2 = bl2[1], r1 = tr[1], r2 = tr2[1];
            if (b1 >= t2 || b2 >= t1 || l1 >= r2 || l2 >= r1) continue;
            let b = Math.max(b1, b2), t = Math.min(t1, t2);
            let l = Math.max(l1, l2), r = Math.min(r1, r2);
            let w = Math.min(t - b, r - l);
