...
```js
/**
 * 计算基于策略调整的最大利润（滑动窗口优化解法）
 * @param {number[]} prices - 每日价格数组
 * @param {number[]} strategy - 初始策略数组（0/1表示是否持有）
 * @param {number} k - 策略调整窗口期
 * @return {number} 可获得的最大利润
 * @note
 * 1. 使用滑动窗口维护策略调整带来的收益变化
 * 2. 前k天分阶段处理策略调整：
 *    - 前k/2天清仓策略
 *    - 后k/2天建仓策略
 * 3. 时间复杂度O(n)，空间复杂度O(1)
...
var maxProfit = function(prices, strategy, k) {
    ...
    // 初始化窗口期策略调整收益
    for (let i = 0; i < k; i++) {
        const weight = i < k/2 ? 0 : 1; // 清仓阶段(0)转建仓阶段(1)
        delta += prices[i] * (weight - strategy[i]);
        sum += prices[i] * strategy[i];
    }

    let max = Math.max(sum, sum + delta);
    
    // 滑动窗口处理后续天数
    for (let i = k; i < n; i++) {
        const outIndex = i - k;       // 移出窗口的索引
        const midIndex = i - k/2;     // 窗口中间索引
        
        // 更新delta：加上新进入元素，减去移出元素和中间元素
        delta += prices[i] * (1 - strategy[i]) 
               - prices[outIndex] * (0 - strategy[outIndex]) 
               - prices[midIndex] * (1 - 0); // 修正中间点的策略变化
