...
4. 遍历前序遍历的剩余元素：
   - 创建新节点
   - 当栈不为空且栈顶节点的值等于中序遍历当前索引的值时：
     - 弹出栈顶节点作为父节点
     - 中序遍历索引递增
   - 如果有父节点，将新节点作为父节点的右子节点
   - 否则，将新节点作为栈顶节点的左子节点
   - 将新节点入栈
5. 返回根节点
...
1. 构建中序遍历值到索引的映射，方便快速查找根节点在中序遍历中的位置
2. 定义递归函数build，参数为前序遍历和中序遍历的边界索引
3. 递归终止条件：如果前序遍历的左边界大于右边界，返回null
4. 从前序遍历的左边界获取根节点的值
5. 在中序遍历中找到根节点的位置，计算左子树的大小
6. 递归构建左子树和右子树
7. 返回根节点
8. 调用build函数，传入整个数组的边界索引
