...
1. **初始化**：建dummy节点，指向head
2. **循环处理**：node从dummy开始
   - 找组：chead=node.next，ctail移动k步
   - 检查：ctail为空则结束
   - 断链：node.next=null，保存next=ctail.next，ctail.next=null
   - 反转：调用reverseList(chead)得newHead和newTail
   - 重连：node.next=newHead，newTail.next=next
   - 移动：node=newTail
3. **返回**：dummy.next

**辅助函数（链表反转）：**

1. **初始**：prev=null，node=head
2. **遍历**：
   - 保存next=node.next
   - 反转指针：node.next=prev
   - 移动：prev=node，node=next
...
var reverseKGroup = function (head, k) {
  ...
  while (node) {
    ...
    let next = ctail.next;
    ctail.next = null;
    const [newHead, newTail] = reverseList(chead);
    ((node.next = newHead), (newTail.next = next));
    node = newTail;
  }
  return dummy.next;
};
