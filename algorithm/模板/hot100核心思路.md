...
1. 初始化结果数组和当前节点curr为根节点
2. 当curr不为空时，循环处理：
   - 如果curr有左子节点，找到左子树的最右节点mostRight：
     - 如果mostRight的右指针为空，将其指向curr，然后curr移到左子节点
     - 如果mostRight的右指针指向curr，说明已经遍历过左子树，记录curr的值，将mostRight的右指针置空，然后curr移到右子节点
   - 如果curr没有左子节点，记录curr的值，然后curr移到右子节点
3. 返回结果数组
...
1. 初始化栈和结果数组，当前节点curr为根节点
2. 当栈不为空或curr不为空时，循环处理：
   - 如果curr不为空，将curr入栈，然后curr移到左子节点
   - 如果curr为空，弹出栈顶节点，记录其值，然后curr移到右子节点
3. 返回结果数组
