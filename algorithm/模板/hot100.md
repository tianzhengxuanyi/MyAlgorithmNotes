...
1. 计算每个位置左侧的最大高度，存储在leftMax数组中
2. 从右向左遍历，计算每个位置右侧的最大高度
3. 对于每个位置，计算其能接的雨水量：左右最大高度的较小值减去当前高度
4. 累加所有位置的雨水量，返回总结果
...
1. 初始化左右指针l=0, r=n-1，以及左右最大高度leftMax=0, rightMax=0
2. 当l <= r时，循环处理：
   - 如果leftMax <= rightMax，处理左指针：
     - 计算当前左指针位置的雨水量并累加
     - 更新leftMax为当前高度和leftMax的较大值
     - 左指针右移
   - 否则，处理右指针：
     - 计算当前右指针位置的雨水量并累加
     - 更新rightMax为当前高度和rightMax的较大值
     - 右指针左移
3. 返回总雨水量
...
1. 计算所有区间的最大右端点，确定差分数组的长度
2. 初始化差分数组，记录区间的开始和结束
3. 遍历差分数组，计算前缀和
4. 根据前缀和的变化，确定合并后的区间：
   - 当前缀和从0变为正数时，开始一个新区间
   - 当前缀和从正数变为0时，结束当前区间
...
2. 初始化结果数组和当前合并区间的起始、结束位置
3. 遍历每个区间：
   - 如果当前区间的起始位置大于当前合并区间的结束位置，说明需要开始一个新区间
   - 否则，更新当前合并区间的结束位置为两者的较大值
4. 将最后一个合并区间添加到结果数组中
5. 返回合并后的区间数组
...
1. 将数组中的非正数替换为n+1（n为数组长度）
2. 遍历数组，对于每个正数（小于等于n），将其对应的下标位置的值标记为负数
3. 再次遍历数组，找到第一个正数的位置，返回其下标+1
4. 如果所有位置都是负数，返回n+1
...
1. 遍历数组的每个元素
2. 对于每个元素，检查是否在1~n范围内，并且是否不在正确的位置上：
   - 正确位置的定义：数字x应该放在索引x-1的位置
   - 使用条件 `nums[i] >= 1 && nums[i] <= n && nums[i] !== nums[nums[i] - 1]` 来判断
   - 解释：`nums[i] !== nums[nums[i] - 1]` 确保我们不会陷入无限循环，同时确保交换是有意义的（避免相同元素交换）
3. 如果元素需要调整位置，将其与正确位置上的元素交换
4. 重复步骤2-3，直到当前元素不需要调整或不在1~n范围内
5. 再次遍历数组，找到第一个位置i，其中nums[i] !== i+1，返回i+1
6. 如果所有位置都正确，返回n+1
...
1. 对矩阵进行转置，将(i, j)位置的元素与(j, i)位置的元素交换
2. 对每一行进行翻转，将行中的元素顺序颠倒
3. 完成旋转90度的操作
...
1. 从外到内逐层处理矩阵，每层对应一个正方形
2. 对于每一层，遍历该层的上边元素
3. 对每个元素，按照顺时针方向依次与右、下、左三个位置的元素交换
4. 完成一层后，缩小边界，处理内层
5. 重复直到所有层都处理完毕
