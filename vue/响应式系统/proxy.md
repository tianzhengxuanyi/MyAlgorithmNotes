...
> 对齐原书PDF，**笔记只保留面试核心重点**；代码为书中最终版实现，可复制运行；不含冗余讲解。
> 格式：每一章分为「📖精简面试笔记」、「💻本章完整最终代码」
...
function toRaw(observed) { return rawMap.get(observed) }
...
function getTargetType(target) {
  ...
  if (['[object Set]','[object Map]','[object WeakSet]','[object WeakMap]'].includes(t)) return 2
...
function reactive(target) { return createReactive(target, false, false) }
function shallowReactive(target) { return createReactive(target, true, false) }
function readonly(target) { return createReactive(target, false, true) }
function shallowReadonly(target) { return createReactive(target, true, true) }
```
...
3. **自动脱ref（unref）**
    ...
    - 注意：**对象属性不会自动脱ref**；`const obj = {a:ref(1)}`，模板`{{obj.a}}`，仍要`.value`。
4. **API区分**
    | API | 作用 |
    |-----|------|
...
> 💻提示：第8章代码量巨大，书中重点在于分支判断；完整diff算法进入第9‑11章（简单diff、双端diff、快速diff）。
