...
- 新的 VNode 数组的长度大于旧的 VNode 数组的长度，那么就会在末尾挂在对应节点。![挂载新节点](../image/挂载新节点.png)
- 新的 VNode 数组的长度小于旧的 VNode 数组的长度，那么就会卸载多余的节点。![卸载节点](../image/卸载节点.png)
...
- 当顺序变化时
  ![新旧子节点位置](../image/新旧子节点位置.png)
  新节点在旧节点中的索引位置按顺序分别为[2,0,1]，不具有递增性，所以需要移动节点。

**lastIndex 减少移动的原理：**

按新子节点顺序从左到右遍历，用 `lastIndex` 记录「已处理节点里最大的旧下标」。

- 当前节点旧下标 `j >= lastIndex`：相对顺序仍递增，DOM 位置本来就对，只 patch，并令 `lastIndex = j`
- 当前节点旧下标 `j < lastIndex`：相对前面已处理节点发生了逆序，才把 DOM 插到前一个新节点后面

直觉：递增序列本身已有序，不必动；只有「本该更靠后、却出现在前面」的节点才移动。例如旧索引序列 `[0,1,2]` 零次移动，`[2,0,1]` 里 `0`、`1` 各因小于 `lastIndex=2` 而移动一次。
