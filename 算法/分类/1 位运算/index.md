...
var hammingWeight = function (n) {
  let ans = 0; // 计数器，记录1的个数
  while (n) {
    // 当n不等于0时循环
    ans++;
    n &= n - 1; // 关键操作：消除二进制表示中最右边的1
...
[Integer.bitCount() 函数理解（尽量通俗易懂）](https://blog.csdn.net/qq_27007509/article/details/112246576)
...
var hammingWeight = function (n) {
  ...
  // 等价写法：i = (i & 0x55555555) + ((i >>> 1) & 0x55555555);
  // 第一行代码的加号前求得偶数区间的 1，加号后求得奇数区间的 1，相加等于合并了相邻的「奇偶区间」，把 区间缩小为 4 个。
  n = n - ((n >> 1) & 0x55555555);

  // 第二步：每4位为一组，将相邻两组的统计结果相加
  // 0x33333333 = 00110011...0011
  n = (n & 0x33333333) + ((n >> 2) & 0x33333333);

  // 第三步：每8位为一组，将相邻四组的统计结果相加
  // 0x0F0F0F0F = 00001111...00001111
  // 然后通过乘以0x1010101将所有8位的统计结果累加到最高8位
  // 最后右移24位得到最终结果
  // (n + (n >> 4) & 0x0F0F0F0F 等价 i = (i & 0x0f0f0f0f) + ((i >>> 4) & 0x0f0f0f0f);
  return (((n + (n >> 4)) & 0x0f0f0f0f) * 0x1010101) >> 24;

  // 等价写法：将所有8位的统计结果累加到最低8位 32位所以0x3f就能满足要求
  // i = i + (i >>> 8);
  //  i = i + (i >>> 16);
  //  return i & 0x3f; // 0x3f = 00...00111111
};
```
...
var hammingWeight = function(n) {
    // 第一步：每2位为一组，统计每组中1的个数
    // 0x55555555 = 01010101...0101 (二进制)
    // 将n与掩码相与得到偶数位的1，右移1位后与掩码相与得到奇数位的1，相加合并
    n = (n & 0x55555555) + ((n >> 1) & 0x55555555);
    
    // 第二步：每4位为一组，将相邻两组的统计结果相加
    // 0x33333333 = 00110011...0011 (二进制)
    // 将每4位中的前2位和后2位的统计结果相加
    n = (n & 0x33333333) + ((n >> 2) & 0x33333333);
    
    // 第三步：每8位为一组，将相邻四组的统计结果相加
    // 0x0F0F0F0F = 00001111...00001111 (二进制)
    // 将每8位中的前4位和后4位的统计结果相加
    n = (n & 0x0F0F0F0F) + ((n >> 4) & 0x0F0F0F0F);
    
    // 第四步：将高8位和低8位的统计结果累加
    n = n + (n >> 8);
    
    // 第五步：将高16位和低16位的统计结果累加
    n = n + (n >> 16);
    
    // 最终结果取低6位（因为32位数最多32个1，6位足够表示）
