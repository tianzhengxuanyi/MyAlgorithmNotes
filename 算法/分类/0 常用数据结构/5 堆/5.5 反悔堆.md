...
/**
 * 计算使用砖块和梯子能到达的最远建筑索引
 * @param {number[]} heights - 建筑物高度数组
 * @param {number} bricks - 可用砖块总数
 * @param {number} ladders - 可用梯子总数
 * @returns {number} 能到达的最远建筑索引
 * @complexity 时间复杂度 O(n log k)，n为建筑数量，k为梯子数量
 * @algorithm 反悔贪心算法：用最大堆记录已使用的砖块，必要时用梯子替换最大砖块消耗
 */
var furthestBuilding = function (heights, bricks, ladders) {
    // 最大堆记录已使用的砖块量（用于后续可能的替换）
    ...
    for (; i < heights.length - 1; i++) {
        ...
        if (ladders > 0) {
            ladders--;
            bricks += pq.dequeue();  // 取回最大的砖块消耗
        } else {
            return i;  // 无法继续前进
        }
    }
    return i;  // 成功到达的最后一个建筑
};
