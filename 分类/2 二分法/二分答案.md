...
var minimizeMax = function(nums, p) {
    ...
    // 排序后相邻元素的差值最小，便于寻找最优解
    nums.sort((a, b) => a - b);
    const n = nums.length, d = Array(n - 1);
    
    // 初始化二分边界：left为最小差值，right为最大差值
    let left = Infinity, right = -Infinity;
    for (let i = 1; i < n; i++) {
        d[i-1] = nums[i] - nums[i-1];
        left = Math.min(left, d[i-1]);
        right = Math.max(right, d[i-1]);
    }

    // 检查是否能在允许最大差值为m的情况下选出p个数对
    const check = (m) => {
        let cnt = 0, prePicker; // prePicker记录上一个选中的索引
        for (let i = 0; i < n - 1; i++) {
            // 当前差值符合要求，且不与前一个选中的索引相邻（避免重叠）
            if (d[i] <= m && i - 1 !== prePicker) {
                cnt++;
                prePicker = i; // 记录当前选中的索引
            }
            // 提前终止：已找到足够数对
            if (cnt >= p) return false; 
        }
        return cnt < p; // 返回是否不满足条件
    }

    // 二分查找最小化最大差值
    while (left <= right) {
        let mid = Math.floor((right - left) / 2) + left;
        // check返回true说明mid太小，需要增大下界
        if (check(mid)) {
            left = mid + 1;
        } else {
            right = mid - 1;
    ...
    return left; // 最终left指向满足条件的最小最大值
};
 ...
 */
var minimizeMax = function (nums, p) {
    // 边界情况处理：不需要选择数对时直接返回0
    if (p == 0) return 0;
    
    // 排序后计算相邻差值，并对差值数组排序（优化二分过程）
    nums.sort((a, b) => a - b);
    const n = nums.length, d = Array(n - 1);
    for (let i = 1; i < n; i++) {
        d[i - 1] = nums[i] - nums[i - 1];
    }
    d.sort((a, b) => a - b); // 差值排序后便于二分查找

    // 检查是否能用排序后的第mid个差值作为最大差值
    const check = (m) => {
        let cnt = 0, prePicker; // prePicker记录上一个选中的索引
        for (let i = 1; i < n; i++) {
            // 当前差值符合要求，且不与前一个选中的索引相邻（避免重叠）
            if ((nums[i] - nums[i - 1]) <= m && i - 1 !== prePicker) {
                cnt++;
                prePicker = i; // 更新最后选中的索引
            }
            if (cnt >= p) return false; // 提前终止条件
        }
        return cnt < p; // 返回是否不满足条件
    }

    // 在排序后的差值数组上进行二分查找
    let left = 0, right = n - 2; // d数组的有效索引范围是[0, n-2]
    while (left <= right) {
        let mid = Math.floor((right - left) / 2) + left;
        // check返回true说明d[mid]太小，需要增大下界
        if (check(d[mid])) {
            left = mid + 1;
        } else {
            right = mid - 1;
