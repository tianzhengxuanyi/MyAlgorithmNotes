...
dfs(i, j, k)表示s[i...j]，最多k次操作的最长回文子序列长度。

状态转移：
- 当s[i] === s[j]时，dp[i][j][k] = dp[i+1][j-1][k] + 2。
- 当s[i]!== s[j]时，dp[i][j][k] = max(dp[i+1][j][k], dp[i][j-1][k], dp[i+1][j-1][k], dp[i+1][j-1][k - ops] + 2)。
  - dp[i+1][j][k] 表示不选择s[i]
  - dp[i][j-1][k] 表示不选择s[j]
  - dp[i+1][j-1][k] 表示不选择s[i]和s[j]
  - dp[i+1][j-1][k-ops] + 2 表示选择s[i]和s[j], 其中ops为使s[i]和s[j]相等所需的最小操作次数
    - d = Math.abs(s[i].charCodeAt() - s[j].charCodeAt()), ops = Math.min(d, 26 - d)。
- 当s[i] === s[j]时，ops === 0，所有可以用dp[i+1][j-1][k - ops] + 2表示，两种状态合并
...
var longestPalindromicSubsequence = function (s, k) {
    const n = s.length;
    // 三维记忆数组：memo[i][j][r] 表示区间[i,j]在剩余r次操作时的最长回文子序列
    const memo = Array.from({ length: n }, () => 
        Array.from({ length: n }, () => Array(k + 1).fill(-1))
    )

    // DFS递归函数：计算区间[i,j]在剩余r次操作时的最大回文长度
    const dfs = (i, j, r) => {
        if (r < 0) return 0;        // 操作次数耗尽
        if (i > j) return 0;        // 无效区间
        if (i == j) return memo[i][j][r] = 1; // 单字符本身就是回文
        if (memo[i][j][r] >= 0) return memo[i][j][r]; // 记忆化返回
        
        // 计算两端字符的转换成本
        let codeI = s[i].charCodeAt(),
            codeJ = s[j].charCodeAt();
        let d = Math.abs(codeI - codeJ);
        let ops = Math.min(d, 26 - d); // 字母表循环特性（比如a-z只需1次操作）

        // 三种基础策略取最大值
        let res = Math.max(
            dfs(i + 1, j, r),      // 舍弃左端字符
            dfs(i, j - 1, r),      // 舍弃右端字符
            dfs(i + 1, j - 1, r)   // 同时舍弃两端字符
        )

        // 如果剩余操作足够让两端字符相等，尝试该策略
        if (ops <= r) {
            res = Math.max(res, 
                dfs(i + 1, j - 1, r - ops) + 2 // 消耗ops次操作使两端相等，长度+2
            )
        }
        return memo[i][j][r] = res; // 存储计算结果
    };
