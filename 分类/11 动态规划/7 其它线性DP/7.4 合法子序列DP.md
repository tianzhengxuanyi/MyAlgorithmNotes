...
- dp[m] 表示在`(sub[x - 2] + sub[x - 1]) % k = i`时， 以 余数为m 的数结尾的最长子序列长度，dp[m] = dp[(i - m + k) % k] + 1
...
#### [940. 不同的子序列 II ☆](https://leetcode.cn/problems/distinct-subsequences-ii/description/)
...
**[题解](https://leetcode.cn/problems/distinct-subsequences-ii/solutions/1890716/xi-fen-wen-ti-fu-za-du-you-hua-pythonjav-1ihu/)**

**动态规划：** 
- dp[i][x] 表示以s[0:i]的所有子序列中，以字符x结尾的子序列个数。
- dp[i][x] = 1 + sum(dp[i-1][y]) (y为a-z中所有字符)
