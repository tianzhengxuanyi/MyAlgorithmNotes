...
/**
 * 计算满足以下条件的整数数量：
 * 1. 该整数在 num1 和 num2 所表示的数值范围之内（包含边界）。
 * 2. 该整数的各位数字之和在 min_sum 和 max_sum 范围之内（包含边界）。
 * @param {string} num1 - 表示范围下限的数字字符串。
 * @param {string} num2 - 表示范围上限的数字字符串。
 * @param {number} min_sum - 各位数字之和的下限。
 * @param {number} max_sum - 各位数字之和的上限。
 * @return {number} - 满足条件的整数数量对 1e9 + 7 取模的结果。
 */
var count = function (num1, num2, min_sum, max_sum) {
    // 获取 num2 的长度，作为统一后的数字长度
    const len = num2.length;
    // 为 num1 补前导零，使其长度与 num2 一致
    num1 = "0".repeat(len - num1.length) + num1;

    // 初始化记忆化数组，用于存储不受上下限限制时的计算结果
    // 第一维表示当前处理的位数，第二维表示当前数字的各位数字之和
    const memo = Array.from({ length: len }, () => Array(401).fill(-1));
    /**
     * 深度优先搜索函数，计算从第 i 位开始满足条件的数字数量
     * @param {number} i - 当前处理的位数索引。
     * @param {number} sum - 当前已经累加的各位数字之和。
     * @param {boolean} isLoLimit - 表示当前是否受到 num1 的下限约束。
     * @param {boolean} isHiLimit - 表示当前是否受到 num2 的上限约束。
     * @return {number} - 从第 i 位开始满足条件的数字数量对 1e9 + 7 取模的结果。
     */
    const dfs = (i, sum, isLoLimit, isHiLimit) => {
        // 如果当前数字之和已经超过 max_sum，不满足条件，返回 0
        if (sum > max_sum) return 0;
        // 当处理完所有位数时，检查数字之和是否在 [min_sum, max_sum] 范围内
        if (i == len) return min_sum <= sum ? 1 : 0;
        // 如果当前不受上下限限制且之前已经计算过该状态，直接返回记忆化结果
        if (!isLoLimit && !isHiLimit && memo[i][sum] >= 0) return memo[i][sum];
        // 初始化满足条件的数字数量
        let res = 0;

        // 确定当前位可填数字的上限，受上限约束时取 num2 对应位，否则为 9
        let hi = isHiLimit ? num2[i] - 0 : 9;
        // 确定当前位可填数字的下限，受下限约束时取 num1 对应位，否则为 0
        let lo = isLoLimit ? num1[i] - 0 : 0;

        // 枚举当前位可填的数字
        for (let j = lo; j <= hi; j++) {
            // 递归处理下一位，更新数字之和以及上下限约束状态，并对结果取模
            res = (res + dfs(i + 1, sum + j, isLoLimit && j == num1[i], isHiLimit && j == num2[i])) % MOD;
        }
        // 如果当前不受上下限限制，将结果存入记忆化数组
        if (!isLoLimit && !isHiLimit) {
            memo[i][sum] = res;
        }
    ...
    // 从第 0 位开始递归，初始状态为受上下限限制，数字之和为 0
