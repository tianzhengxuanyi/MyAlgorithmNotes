...
var countBalls = function(lowLimit, highLimit) {
    ...
    // 深度优先搜索函数
    const dfs = (i, r, isLowLimit, isHihgLimit) => {
        // 递归终止条件：处理完所有位数后检查是否达到目标数位和
        if (i === len) return r === 0 ? 1 : 0;
        // 检查是否可以使用记忆化结果（不受限制的情况）
        if (!isHihgLimit && !isLowLimit && memo[i][r] >= 0) return memo[i][r];
        
        // 确定当前位可填数字的上下界
        const hi = isHihgLimit ? high[i] : 9;  // 受上限约束时取high对应位，否则9
        ...
        for (let j = lo; j <= hi; j++) {
            ...
            res += dfs(i + 1, r - j, isLowLimit && j == lo, isHihgLimit && j == hi);
        }
        
        // 记忆化不受限制时的结果
        if (!isHihgLimit && !isLowLimit) memo[i][r] = res;
        return res;
    }
