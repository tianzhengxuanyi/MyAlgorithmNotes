...
/**
 * 递归生成所有有效的单行涂色状态
 * @param {number} i - 当前处理的列索引(0-2)
 * @param {number} path - 当前状态路径（二进制位表示）
 */
const getProb = (i, path) => {
  if (i == 3) {
    // 已处理完3列
    prob.push(path); // 保存有效状态
    return;
  }
