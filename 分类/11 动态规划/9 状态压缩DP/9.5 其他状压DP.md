...
- f*i_ABC = 2 * f*i-1_ABC + 2 * f_i-1_ABA
- f*i_ABA = 2 * f*i-1_ABC + 3 * f_i-1_ABA
...
var canPartitionKSubsets = function (nums, k) {
  ...
  // 记忆化数组，memo[s]表示状态s是否能被成功划分
  // 状态s是一个二进制数，每一位表示对应数字是否被使用
  // ⭐当前可选状态确定后，对应的t值也是确定的(sum(s) % part)，所以不用缓存t
  const memo = Array(1 << n);

  /**
   * 深度优先搜索函数
   * @param {number} s - 当前状态，二进制位表示数字是否被使用
   * @param {number} t - 当前子集已累加的和
   * @return {boolean} - 是否能成功划分
   */
  const dfs = (s, t) => {
    // 基本情况：所有数字都已使用，说明成功划分
    if (s == 0) return true;
    // 如果当前状态已计算过，直接返回结果
    ...
    for (let j = 0; j < n; j++) {
      // 跳过已使用的数字
      if (!(s & (1 << j))) continue;
      // 剪枝：如果当前数字会使和超过part，后面的数字更大，直接跳出循环
      if (t + nums[j] > part) break;

      // 递归尝试选择当前数字
      // s ^ (1 << j) 表示将第j位设为0（已使用）
      // (t + nums[j]) % part 计算新的累加和（模part处理）
      if (dfs(s ^ (1 << j), (t + nums[j]) % part)) {
        return (memo[s] = true);
      }
    }
