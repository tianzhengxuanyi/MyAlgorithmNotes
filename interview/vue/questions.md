...
11. **forceUpdate**
   在 Vue 3 中，$forceUpdate 的原理是绕过响应式依赖收集的比较，直接通过调度器强制将组件的更新任务推入异步队列，从而触发重新渲染。其源码主要位于 packages/runtime-core/src/componentPublicInstance.ts。`$forceUpdate: i => i.f || (i.f = () => queueJob(i.update)),`
