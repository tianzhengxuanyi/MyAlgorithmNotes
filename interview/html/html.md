...
## [12、浏览器从输入URL到页面渲染加载的过程](https://juejin.cn/post/7316775422187061300)

当在浏览器输入 URL 并按下回车，从请求发起至页面完整渲染，完整流程如下：

1. **启动渲染进程**
   浏览器为当前标签页分配独立渲染进程，该进程内部包含 GUI 渲染线程、JS 引擎线程、事件线程、定时器线程、异步请求线程。其中JS 引擎单线程运行，为避免 DOM 操作冲突，JS 线程和 GUI 渲染线程互斥；页面解析遇到 JS 文件会阻塞 HTML 解析，可通过 defer/async 优化加载执行时机。

2. **解析 URL**
   浏览器解析统一资源定位符，拆分出协议、主机名、端口、请求路径、查询参数、锚点等部分。

3. **DNS 域名解析（域名→IP）**
   按照缓存逐级查询：先查浏览器缓存 → 系统 Host 文件 → 本地 DNS 服务器缓存；无结果则向上查询根域名服务器、顶级域名服务器、权威域名服务器，最终获取服务器 IP，并逐级缓存解析结果。可使用 [dns-prefetch](https://jishuzhan.net/article/2056648330457485314) 提前解析优化首屏。

4. **基于 TCP 协议建立连接**
   基于 TCP/IP 四层网络模型（应用层、传输层、网络层、数据链路层），通过三次握手确认双方收发能力正常，建立可靠 TCP 连接。

5. **发起 HTTP 请求并接收响应**
   - 浏览器组装 HTTP 请求报文（请求行、请求头、请求体）发送至服务端；
   - 服务处理后返回响应报文，包含状态行、响应头、响应体；
   - 流程中会使用HTTP 强缓存、协商缓存提升加载速度，同时通过 Cookie 完成身份交互；
   - 区分 HTTP/1.0、HTTP/1.1、HTTP/2、HTTP/3 的特性差异；
   - 连接根据 Connection 字段判断是否为长连接。

6. **关闭 TCP 连接**
   数据传输完毕后，TCP 全双工通道需要四次挥手分别关闭两个方向的数据流，客户端会等待 2MSL 确保对方正常断开。

7. **浏览器解析并渲染页面**
   - 解析 HTML 字节流，生成 Token，最终构建DOM 树；
   - 并行解析 CSS，生成CSSOM 树；
   - 合并 DOM 与 CSSOM，过滤不可见节点，生成渲染树；
   - 布局（回流）：计算节点位置、尺寸，生成布局树；
   - 分层：根据堆叠上下文、transform 等对页面分层；
   - 绘制：分层完成像素绘制，最终展示页面。
...
JS 设计初衷是操作 DOM，多线程同时修改 DOM 会引发数据冲突、逻辑混乱，因此设计为单线程；依靠事件循环、异步线程实现并发能力。

#### 2. GUI 线程和 JS 线程为什么互斥？

JS 可以修改 DOM 与样式，若两者并行执行，会导致渲染结果不一致，因此 JS 执行时渲染线程挂起，JS 空闲后再执行渲染。

#### 3. defer 和 async 区别？

- **defer**：JS 文件异步下载，DOM 解析完成后按顺序执行
- **async**：JS 文件异步下载，下载完成立即执行，执行顺序不可控
- 二者均不会阻塞 HTML 解析

#### 4. 为什么 TCP 是三次握手、四次挥手？

- **三次握手**：确认客户端、服务端双方收发能力都正常，防止失效旧报文造成服务器资源浪费
- **四次挥手**：TCP 是全双工通信，两个方向数据流需要分别关闭，服务端可能存在剩余数据要传输，因此无法合并挥手步骤

#### 5. 强缓存和协商缓存区别？

- **强缓存**：命中后不请求服务器，直接使用本地缓存，由 Expires、Cache-Control 控制
- **协商缓存**：强缓存失效后，携带 Last-Modified/ETag 向服务器校验，资源无更新返回 304，使用本地缓存；有更新则返回新资源

#### 6. 回流和重绘区别？

- **回流（重排）**：元素布局、尺寸、位置发生改变，重新计算布局树，性能开销大
- **重绘**：仅元素外观样式改变，布局不变，仅重新绘制像素，开销更小
- **规则**：回流一定会触发重绘，重绘不会触发回流
